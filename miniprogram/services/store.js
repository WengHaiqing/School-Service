...
function createTask(payload) {
  ...
  const task = {
    ...
    status: 'open',
    verified: true,
    expiresAt: iso(Number(payload.expiryHours) * 3600000),
    serviceDueAt: iso(Number(payload.serviceHours) * 3600000),
    extensionCount: 0,
    createdAt,
...
function addMessage(id, content) {
  ...
  order.messages.push({ id: `m_${Date.now()}`, senderId: state.currentUserId, content: value, createdAt: iso() })
  write(state)
  return decorateOrder(state, order)
}
